...
import { CircleDot, Filter, LayoutGrid, Plus, Star } from "lucide-react";
import Loading from "../../components/Loading";

const boardFields = [
  { name: "image", label: "Board Image", type: "file" },
  ...
  { name: "featured", label: "Featured Board?", type: "checkbox" },
];
...
const Boards = () => {
  const { data, loading, createItem, updateItem, deleteItem } = useCrud("/boards");
  ...
  useEffect(() => {
    data.forEach((board) => {
      ...
      if (dayDiff >= 0 && dayDiff <= 10) {
        toast.warning(
          `${board.location} will be available in ${dayDiff} day${dayDiff === 1 ? "" : "s"
          }.`
        );
  ...
  const openForEdit = (row) => {
    setEditing({
      ...
      availableDate: row.availableDate
        ? row.availableDate.slice(0, 16)
        : "",
      image: null, //  reset image so user can upload new
    });
    setIsOpen(true);
  };
  ...
  const handleSave = async (form) => {
    ...
    //  safe append (important)
    Object.entries(form).forEach(([key, value]) => {
  

      // convert boolean → string for backend safety
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Unable to save board."
      );
      throw error;
    }
  };
  ...
  if(loading) {
    return (
        ...
        <Loading message="Loading Boards"/>
    ...
    <div className="space-y-5">
      <section className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Track billboard inventory, status and pricing.
          </p>
        </div>
      ...
      <Table
        columns={[
          ...
          { key: "size", label: "Size" },
          // show a star for featured boards with the location
          {
            key: "location",
            label: "Location",
            render: (row) => (
